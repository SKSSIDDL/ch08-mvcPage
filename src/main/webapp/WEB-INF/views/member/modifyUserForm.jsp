...
<script type="text/javascript">
	$(function(){
		let idChecked = 0; //0은 중복체크 미실행 또는 중복 된 경우, 1은 미중복
		//아이디 중복체크
		...
		$('#register_form #id').keydown(function(){ //=$('#id')
			idChecked = 0;
			$('#message_id').text(''); //검색하지 않은 것 처럼 초기화
		}); //end of keydown
		...
		$('#register_form').submit(function(){
			...
			for(let i=0; i<items.length; i++){
				...
				if(items[i].id == 'id' && idCheck==0){
					alert('ID 중복체크 필수');
					return false;
				}
				
				if(items[i].id == 'zipcod' && !/^[0-9]{5}$/.text($('#zipcode').val())){
			...
			<form id="modify_form" action="modifyUser.do" method="post">
				<ul>
					...
					<li>
						...
						<input type="text" name="address1" id="address1" maxlength="30" value="${member.address1}" class="input-check">
					</li>
					<li>
						<label for="address2">상세주소</label>
						<input type="text" name="address2" id="address2" maxlength="30" value="${member.address2}" class="input-check">
					</li>
				</ul>
